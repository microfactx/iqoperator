...
function shortTime(raw: any): string {
  ...
  const s = String(raw)
  // Tenta extrair HH:MM(:SS) se houver, senão trunca
...
function CustomTooltip({
  ...
  return (
    ...
    >
      <div style={{ color: "#E6E6E6", fontWeight: 600, marginBottom: 4 }}>
        Trade #{p.idx}
      </div>
        ...
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={data} margin={{ top: 5, right: 12, left: 0, bottom: 5 }}>
            ...
            <XAxis
              ...
              stroke="#8A8F98"
              tick={{ fontSize: 11 }}
              tickFormatter={(v: string, i: number) => shortTime(data[i]?.time) || String(v)}
