"""Hunter EURUSD M15 histdata - Bollinger p=20,50 mult=1.5,2.0 exp=15/30m e Donchian n=20,40 exp=15/30m.
Payout 0.90 BE 52.63%. IS 70% OOS 30%. Retorne top 3 IS e veredito OOS."""
import csv
import math
import sys
import os
...
def main():
    d = load_m15("data/EURUSD_M15_histdata.csv")
    c = d["c"]
    n_total = len(c)
    cut = int(n_total * 0.70)
    print(f"[DATA] EURUSD_M15_histdata: {n_total} M15 | IS:0..{cut-1} ({cut}) OOS:{cut}..{n_total-1} ({n_total-cut})")
    ...
    print(f"\n=== TOP 3 IS por LUCRO ===")
    print(
        f"{'#':<3} {'cfg':<35} {'IS profit':<10} {'IS n':<6} {'IS WR':<7} {'IS Wilson':<10} {'OOS profit':<11} {'OOS n':<6} {'OOS WR':<7} {'OOS Wilson':<11} {'vs BE':<9} {'OOS verdict'}"
    )
    for idx, r in enumerate(rows_sorted[:3], 1):
        ...
        oos = r["r_oos"]
        print(
            f"{idx:<3} {cfg:<35} {is_['profit']:+8.2f}  {is_['trades']:<6} {is_['wr']:.2%}  {r['w_is']:.2%}     {oos['profit']:+8.2f}   {oos['trades']:<6} {oos['wr']:.2%}  {r['w_oos']:.2%}     {r['w_oos']-r['BE']:+.2%}   {'PASS' if r['ok_oos'] else 'FAIL'}"
        )
    ...
    print(
        f"{'#':<3} {'cfg':<35} {'OOS Wilson':<11} {'OOS n':<6} {'OOS WR':<7} {'OOS profit':<11} {'vs BE':<9} {'IS profit':<10} {'IS Wilson':<10} {'OOS verdict'}"
    )
    for idx, r in enumerate(rows_oos_wilson[:3], 1):
        if r["type"] == "boll":
            ...
            cfg = f"donchian n={r['n']} exp={r['exp_m']}m"
        print(
            f"{idx:<3} {cfg:<35} {r['w_oos']:.2%}       {r['r_oos']['trades']:<6} {r['r_oos']['wr']:.2%}  {r['r_oos']['profit']:+8.2f}   {r['w_oos']-r['BE']:+.2%}   {r['r_is']['profit']:+8.2f}   {r['w_is']:.2%}     {'PASS' if r['ok_oos'] else 'FAIL'}"
        )
    ...
    print(
        f"{'#':<3} {'cfg':<35} {'OOS profit':<11} {'OOS n':<6} {'OOS WR':<7} {'OOS Wilson':<11} {'IS profit':<10} {'IS WR':<7}"
    )
    for idx, r in enumerate(rows_oos_profit[:3], 1):
        if r["type"] == "boll":
            ...
            cfg = f"donchian n={r['n']} exp={r['exp_m']}m"
        print(
            f"{idx:<3} {cfg:<35} {r['r_oos']['profit']:+8.2f}   {r['r_oos']['trades']:<6} {r['r_oos']['wr']:.2%}  {r['w_oos']:.2%}     {r['r_is']['profit']:+8.2f}   {r['r_is']['wr']:.2%}"
        )
    ...
    print(f"Configs testadas: {len(rows)} (Bollinger: {len(P_LIST)*len(MULT_LIST)*len(EXPIRIES)}, Donchian: {len(N_LIST)*len(EXPIRIES)})")
